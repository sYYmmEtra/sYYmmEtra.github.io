...
describe("publication wrappers", () => {
  it("uses physical self-directory, fail-fast zsh, and separate sync and publish boundaries", async () => {
    const root = path.resolve(".");
    const publish = await fs.readFile(path.join(root, "发布个人博客.command"), "utf8");
    const syncAndPublish = await fs.readFile(path.join(root, "同步并发布个人博客.command"), "utf8");
    for (const wrapper of [publish, syncAndPublish]) {
      expect(wrapper).toContain("#!/bin/zsh");
      expect(wrapper).toContain("set -euo pipefail");
      expect(wrapper).toContain('script_dir="${0:A:h}"');
      expect(wrapper).toContain('cd "$script_dir"');
    }
    expect(publish).toMatch(/^npm run publish$/m);
    expect(syncAndPublish).toMatch(/^\.\/同步博客内容\.command$/m);
    expect(syncAndPublish).toMatch(/^npm run publish$/m);
    expect(syncAndPublish).not.toContain("--allow-removals");
  });

  it("uses the sync wrapper's default source and does not publish after sync failure", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "blog-publish-wrapper-"));
    temporaryRoots.push(root);
    const bin = path.join(root, "bin");
    const log = path.join(root, "calls.log");
    await fs.mkdir(bin);
    await fs.copyFile(path.resolve("同步并发布个人博客.command"), path.join(root, "同步并发布个人博客.command"));
    await fs.copyFile(path.resolve("同步博客内容.command"), path.join(root, "同步博客内容.command"));
    await fs.writeFile(path.join(bin, "npm"), `#!/bin/zsh\necho "$1:$2:$AI_DAILY_SOURCE" >> ${JSON.stringify(log)}\nif [[ "$1:$2" == "run:sync" && "${"$FAIL_SYNC"}" == "1" ]]; then exit 7; fi\n`);
    await fs.chmod(path.join(bin, "npm"), 0o755);
    const { AI_DAILY_SOURCE: _testSource, ...environmentWithoutSource } = process.env;
    const environment = { ...environmentWithoutSource, PATH: `${bin}:${process.env.PATH}` };
    await execFileAsync("/bin/zsh", ["-f", path.join(root, "同步并发布个人博客.command")], { env: environment });
    expect(await fs.readFile(log, "utf8")).toBe(
      "run:sync:/path/to/ai-daily\nrun:publish:\n",
    );
    await fs.writeFile(log, "");
    await expect(execFileAsync("/bin/zsh", ["-f", path.join(root, "同步并发布个人博客.command")], {
      env: { ...environment, FAIL_SYNC: "1" },
    })).rejects.toMatchObject({ code: 7 });
    expect(await fs.readFile(log, "utf8")).toBe("run:sync:/path/to/ai-daily\n");
  });
});
