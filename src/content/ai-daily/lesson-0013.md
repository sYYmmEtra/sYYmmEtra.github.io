...
summaryZh: 复访day4 L2→L3(编译+格式税+路由)。JSON嵌套非正则语言,须CFG/下推自动机PDA才能记住括号深度(非纯FSM);管线=schema→一次性编译(20-50ms)出PDA→逐token把非法logit置−∞;xgrammar靠vocab partitioning+mask缓存把per-token开销压到近0(命中后有时比无约束还快)。格式税机制:grammar mask会在某些位置吃掉"该调用工具"的token,模型塌陷成安全默认值(真实案例schema合法99.4%但11%语义跑偏)——schema只保证格式不保证质量。MCP工具膨胀(50工具=2-4万token prefill+选错率升)把"塞哪些工具schema"变成day7/10检索问题,history-aware路由器是新解法
